...
# In this section I generate data, fit models, posterior predictive check, and parameter recovery check results.
# In this fixed data set, there are eight synthetic participants, one of which is difficult to fit (Subject #3)
# Usually, across more sets of eight (or more participants in general), Subject #3 seems to be more of an exception
# than a rule.
#
# Software used:
...
saveRDS( object = list( fit_many_indi ), file = here("fake_data_fits.rds") )
...
# Could be made faster by vectorising the code in 'ExGaussian_fake_data_simulation.R', however, I was too lazy to do so
# because the simulation function works via conducting a simulated experiment one trial after another and the PPC does
# not take prohibitively large amount of time (usually, I just make a tea while running it :-)
...
# not using bayesplot because it's function cannot use vectors with NAs (which I use for PPC)
